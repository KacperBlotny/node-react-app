...
const registerActivity = asyncHandler(async (req, res) => {
  ...
  try {
    // Add to database
    const insertQuery =
      'INSERT INTO Activities (userid, startTime, endTime, name) VALUES ($1, $2, $3, $4) RETURNING activityid'
    const insertValues = [userid, startTime, endTime, name]
...
const editActivity = asyncHandler(async (req, res) => {
  const { userid, name, startTime, endTime, activityid } = req.body
  try {
    const query =
      'UPDATE Activities SET userid = $1,  name = $2, startTime = $3, endTime = $4 WHERE activityid = $5'
    const values = [userid, name, startTime, endTime, activityid]
